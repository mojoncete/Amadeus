...
; Construyo nº aleatorio. Método: "DERIVANDO."
...
;                                       ; El registro R, es un registro de 8 bits que actúa como contador de refresco de la memoria dinámica. ($00 - $ff).
    ld c,0                              ; C contendrá nuestro nº aleatorio: $0 - $ff. Inicialmente está a "0".
    push bc                ;o                     
    ld b,7                              ; B actuará como contador de bits. Requerimos de 1 byte, ($ff).
6 and a                                 ; Carry a "0".
    srl a
    jr nc,1F
    set 0,c    
1 ld d,a                                ; D contiene la copia de nuestra semilla después del DESPLAZAMIENTO.
    ...
    jr 3F
2 xor a
3 xor e                                 ; XOR entre el último y penúltimo bit de A.
    ld a,d
    jr z,4F
    set 7,a
    jr 5F
